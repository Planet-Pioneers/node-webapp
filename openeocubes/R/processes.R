...
load_collection <- Process$new(
  ...
  returns = eo_datacube,
  operation = function(id, spatial_extent, temporal_extent, bands = NULL, resolution = 30, job) {
    ...
    # Mask out pixels classified as cloud or clout shadow by ESA
    ...
    # gdalcubes creation
    # cube <- raster_cube(img.col, v.overview, S2.mask)
    ...
    tryCatch({
      message("\nFill NA pixels of data cube based on time series interpolation...")

      cube.filled = gdalcubes::fill_time(cube, method = "near")

      message("NA pixels filled!")
...
train_model <- Process$new(
  ...
  parameters = list(
    ...
    Parameter$new(
      ...
      description = "GEOJSON with labeled Polygons as a string.",
      schema = list(
        type = "string",
        subtype = "GeoJSON"
      )
    ...
    Parameter$new(
      name = "model_id",
      description = "The id that the model will receive.",
      schema = list(
        type = "string"
      )
    )
  ...
  operation = function(data,labeled_polygons, model_id, job){
    ...
    tryCatch({
      ...
      labeled_polygons = sf::st_read(labeled_polygons)

      # change CRS to cube CRS
      labeled_polygons = sf::st_transform(labeled_polygons, crs = gdalcubes::srs(data))
      # message(labeled_polygons)

      message("Trainingdata sucessfully loaded!")
    },
  error = function(err)
  {
    message("An Error occured!")
    ...
    # extract geometries of polygons from the datacube
  ...
  error = function(err)
  {
    ...
    training_df_filtered$PolygonID = NULL
    training_df_filtered$ClassID = NULL


    print(summary(training_df_filtered))

  tryCatch({
    message("\nRandomly draw pixels")
    trainids = caret::createDataPartition(
      training_df_filtered$Label, list=FALSE, p=0.8
    )
    trainDat = training_df_filtered[trainids,]
    message("reducing data to 80% complete")
      
    predictors <- names(data)
    response <- "Label"
    indices <- CAST::CreateSpacetimeFolds(trainDat, spacevar = "FID", k=3, class = "Label")
    ctrl <- caret::trainControl(method = "cv",
                                index = indices$index)
    model <- CAST::ffs(trainDat[,predictors],
                      trainDat[,response],
                      method="rf",
                      metric="Kappa",
                      trControl=ctrl,
  ...
  error = function(err)
  {
    ...
    tryCatch({
      message("\nSaving model...")
      modelname <- paste0("model", model_id,".rds")
      file_path = file.path(Session$getConfig()$workspace.path, modelname)
      base::saveRDS(model, file_path)
...
#classify
classify <- Process$new(
  id = "classify",
  description = "Classifies a data cube using a model",
  categories = as.array("cubes"),
  summary = "Classifies a data cube using a model",
  parameters = list(
    Parameter$new(
      name = "data",
      description = "Datacube with temporal extent of area of interest",
      schema = list(
        type = "object",
        subtype = "raster-cube"
      )
    ...
    Parameter$new(
      name = "model_id",
      description = "ID of a trained model",
      schema = list(
        type = "string"
      ),
      optional = FALSE
  ...
  returns = eo_datacube,
  operation = function(data, model, model_id, job){
    message("Applying classification...")
    message(model_id)
     tmp = tempdir()
        bandnames <- names(data)
        message("trying to get model from workspace: ")
        modelname <- paste0("model", model_id,".rds")
        file_path <- file.path(Session$getConfig()$workspace.path, modelname)
        message(file_path)
        loaded_model <- readRDS(file_path)
        print(loaded_model)
        message("run apply pixel now....")
        # save this to temporary directory because for some reason in the FUN function it gets lost
        saveRDS(loaded_model, paste0(tmp, "/model.rds"))
        saveRDS(bandnames, paste0(tmp, "/bandnames.rds"))
        #set this temporary directory as environment variable because it won't forget that (hopefully)
        Sys.setenv(TEMP_DIR = tmp)
    ...
    cube = gdalcubes::apply_pixel(
      ...
      names = "prediction",
      FUN = function(x){
         # x : values of the bands from one pixel as a (named) vector 
        # FUN is applied on all pixels 
        #return a numeric vector with identical length for all pixels. 
        #Elements of the result vectors will be interpreted as bands in the result data cube
        message("\nIn the function------------------------------------------------------")
          ...
          tryCatch({
            ...
            model2 = base::readRDS(paste0(tmp, "/model.rds"))
            bandnames = readRDS(paste0(tmp, "/bandnames.rds"))
            named_vector = setNames(x, bandnames)
            
            #make x a dataframe and transform it so it's a row
            x_df = as.data.frame(t(named_vector))
            

            prediction = stats::predict(model2, newdata = x_df)
            message("Prediction")
            print(prediction)
            # Convert the prediction to character
            prediction <- as.character(prediction)
            message("as character:")
            print(prediction)

            # Extract numeric values using regular expressions
            numeric_values <- gregexpr("\\d+", prediction)


            # Extract all matches
            extracted_digits <- regmatches(prediction, numeric_values)
 

            # Convert to numeric vector
            prediction <- as.numeric(unlist(extracted_digits))
            print(prediction)
...
ndvi <- Process$new(
  ...
  parameters = list(
    ...
    Parameter$new(
      name = "keep_bands",
      description = "whether or not to keep the bands of the cobe",
      schema = list(
        type = "boolean"
      ),
      optional = TRUE
  ...
  operation = function(data, nir = "nir", red = "red", keep_bands = FALSE, job){
    ...
    cube <- gdalcubes::apply_pixel(data, ndvi_formula, names = "NDVI", keep_bands = keep_bands)
