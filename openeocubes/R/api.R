...
.models = function() {
  url = Session$getConfig()$workspace.path
  ...
  model_list = list()
   for (file in files) {
        # Load the file using readRDS
        file_path <- file.path(url, file)
        model <- readRDS(file_path)
        output <- capture.output({
          cat(paste(file, "\n"))
          print(model)
        })
         model_list <- c(model_list, list(output))
      }
      message(model_list)
...
addEndpoint = function() {
  Session$createEndpoint(path = "/models",
                         method = "GET",
                         handler = .models)
...
# assign processes
  ...
  Session$assignProcess(classify)
}
